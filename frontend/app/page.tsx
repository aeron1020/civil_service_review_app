...
//         if (data.next) {
//           return fetchAllQuizzes(data.next, combined);
//         }
//         return combined;
//       } catch (error) {
//         console.error("Error fetching quizzes:", error);
...
//           const firstType =
//             allQuizzes[0].quiz_type_display ||
//             allQuizzes[0].quiz_type ||
//             "Other";
...
//   const grouped = quizzes.reduce((acc: any, quiz: any) => {
...
//     cle: "Clerical Ability (for Sub-Professional)",
//   };

//   const selectedTitle =
//     typeTitles[selectedType?.toLowerCase()] || selectedType || "Other";
...
//         Select a category below to explore quizzes based on Civil Service Exam
//         sections.
//       </p>
...
//           <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-6">
//             {grouped[selectedType]?.map((quiz: any) => (
//               <Link
//                 key={quiz.id}
//                 href={`/quiz/${quiz.id}`}
//                 className="glass-card p-5 rounded-xl border border-gray-200/60 dark:border-gray-700/50 shadow-sm hover:shadow-md hover:scale-[1.02] hover:border-[var(--accent)] hover:shadow-[0_4px_20px_rgba(10,132,255,0.25)] transition-all duration-300"
...
type Quiz = {
  ...
  quiz_type: string;
  quiz_type_display?: string;
  questions?: number[]; // include if backend sends question IDs
};
